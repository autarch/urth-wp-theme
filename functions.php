...
function the_bootstrap_register_scripts_styles() {
	...
	if ( ! is_admin() ) {
		...
		wp_register_script(
			'urth',
			get_template_directory_uri() . "/js/urth.js",
			array('the-bootstrap'),
			$theme_version,
			true
		);
		...
		wp_register_style(
			'urth',
			get_template_directory_uri() . "/urth.css",
			array('the-bootstrap'),
			$theme_version
		);
	}
...
function the_bootstrap_comment_form_defaults( $defaults ) {
	return wp_parse_args( array(
		'comment_field'			=>	'<div class="comment-form-comment control-group"><div class="controls"><textarea class="span12" id="comment" name="comment" rows="8" aria-required="true"></textarea><span class="help-block">You may use Markdown or HTML in your comments. Paragraphs will be added automically.</span></div></div>',
